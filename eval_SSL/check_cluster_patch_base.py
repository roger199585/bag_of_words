...
import torchvision.models as models as ssl_model


import dataloaders
from config import ROOT
from preprocess_SSL.SSL import model
...
if __name__ == '__main__':
    ...
    training_features = pickle.load(open(f"{ ROOT }/preprocessData/chunks/ssl/chunks_{ args.data }_train.pickle", "rb"))
    ...
    for idx, img in tqdm(test_normal_loader):
        ...
        for i in range(16):
            for j in range(16):
                if ((i==0 or i ==15) and (j<=4 or j>=11)) or ((i==1 or i ==14) and (j<=2 or j>=13)) or ((i==2 or i==12) and (j<=1 or j>=14)) or ((i==3 or i==4 or i==5 or i==10 or i==11) and (j==0 or j ==15)):
                    patch = img[0, :, i*16:i*16+16, j*16:j*16+16]
                    feature, _ = pretrain_model(patch.unsqueeze(0))
