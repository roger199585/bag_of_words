...
def eval_OriginFeature(pretrain_model, model, test_loader, kmeans, pca, test_data, global_index, good=False):
    ...
    with torch.no_grad():
        ...
        chunk_num = 4 #int(args.image_size / args.patch_size)
        for (idx, img) in test_loader:
            sss = time.time()
            each_pixel_err_sum = np.zeros([256, 256])
            each_pixel_err_count = np.zeros([256, 256])
            ...
            for batch_start_idx in range(0, len(indices), batch_size):
                ...
                for i, j in batch_idxs:
                    ...
                    out = crop_output[0, :, :, :].flatten(0, 1).flatten(0, 1)
...
if __name__ == "__main__":
    ...
    print("----- defect -----")
    if args.resume and os.path.isfile('{}/Results/testing_multiMap/ssl/{}/all/img_all_feature_{}.pickle'.format(ROOT, args.data, args.index)):
        print("load from {}/Results/testing_multiMap/ssl/{}/all/img_all_feature_{}.pickle".format(ROOT, args.data, args.index))
        img_all_feature = pickle.load(open('{}/Results/testing_multiMap/ssl/{}/all/img_all_feature_{}.pickle'.format(ROOT, args.data, args.index), 'rb'))
    else:
    ...
    print("----- good -----")
    if args.resume and os.path.isfile('{}/Results/testing_multiMap/ssl/{}/good/img_good_feature_{}.pickle'.format(ROOT, args.data, args.index)):
        print("load from {}/Results/testing_multiMap/ssl/{}/good/img_good_feature_{}.pickle".format(ROOT, args.data, args.index))
        img_all_feature = pickle.load(open('{}/Results/testing_multiMap/ssl/{}/good/img_good_feature_{}.pickle'.format(ROOT, args.data, args.index), 'rb'))
    else:
    ...
    save_all_path = "{}/Results/testing_multiMap/ssl/{}/all/".format(ROOT, args.data)
    ...
    save_good_path = "{}/Results/testing_multiMap/ssl/{}/good/".format(ROOT, args.data)
    ...
    for ((idx, img), (idx2, img2)) in zip(test_all_loader, mask_loader):
        ...
        errorMap = img_all_feature[idx].reshape((256, 256))
    ...
    for (idx, img) in test_good_loader:
        ...
        errorMap = img_good_feature[idx].reshape((256, 256))
