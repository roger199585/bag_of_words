...
# print(pre_model.features[0].weight)
...
if __name__ == "__main__":
    ...
    """ load dataset """
    train_dataset = MvtecLoader('./dataset/' + args.data + '/train_resize/')
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=False)
    model = model.to(device)
    model.train()
    output_list = []
    # for img in tqdm(train_loader):
    for index, img in tqdm(train_loader):
        img = img.to(device)
        # print(img.size())
        ...
        # input()
        """ split feature """
        # output_data = output.unfold(2, 1, 1).unfold(3, 1, 1).permute(0, 2, 3, 1, 4, 5).contiguous().view(-1, 256, 1, 1)
        output_data = output.unfold(2, 1, 1).unfold(3, 1, 1).contiguous().view(-1, 128, 1, 1)
        output_list.append(output_data.cpu().detach().numpy())

    """ total size of chunks=1024*209 """
    chunks = []
    for i in range(len(output_list)):
        for j in range(len(output_list[i])):
            chunks.append(output_list[i][j])
    
    save_name = 'chunks/resnet34/chunks_' + args.data +'_train.pickle'
    with open(save_name, 'wb') as write:
        pickle.dump(chunks, write)