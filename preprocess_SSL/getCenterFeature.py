...
if args.data == "ALL":
    for idx, data in enumerate(total):
        train_label_name = "{}/preprocessData/label/ssl/{}/train/{}_100.pth".format(ROOT, data, args.kmeans)
        train_label = torch.tensor(torch.load(train_label_name))

        origin_features_path = "{}/preprocessData/chunks/ssl/chunks_{}_train.pickle".format(ROOT, data)
    ...
    train_label_name = f"{ ROOT }/preprocessData/label/ssl/{ args.dim_reduction }/{ args.data }/train/{ args.kmeans }_100.pth"
    train_label = torch.tensor(torch.load(train_label_name))

    origin_features_path = f"{ ROOT }/preprocessData/chunks/ssl/chunks_{ args.data }_train.pickle"
    origin_features = pickle.load(open(origin_features_path, "rb"))
    origin_features = np.array(origin_features)
