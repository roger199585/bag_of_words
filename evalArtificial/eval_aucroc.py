...
## Label
test_label_name = f"{ ROOT }/preprocessData/label/artificial/{ args.data }/test/all_{ args.kmeans }.pth"
test_label      = torch.tensor(torch.load(test_label_name))

test_good_label_name = f"{ ROOT }/preprocessData/label/artificial/{ args.data }/test/good_{ args.kmeans }.pth"
...
for ((idx, img), (idx2, img2)) in zip(test_loader, mask_loader):
    ...
    errorMapPath = f"{ ROOT }/testing/artificial/{ args.data }/all/{ args.kmeans }/"
...
for (idx, img) in test_good_loader:
    ...
    errorMapPath = f"{ ROOT }/testing/artificial/{ args.data }/good/{ args.kmeans }/"
    if not os.path.isdir(errorMapPath):
        os.makedirs(errorMapPath)
        print(f"----- create folder for type:{ args.data } -----")
